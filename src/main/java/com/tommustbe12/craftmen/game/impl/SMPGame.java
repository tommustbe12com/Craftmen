...
public class SMPGame extends Game {
    ...
    public Kit createDefaultKit() {
        ...
        contents[3] = createEnchanted(Material.NETHERITE_AXE, new Object[][]{
                {"sharpness", 5},
                {"efficiency", 5},
                {"unbreaking", 3},
                {"mending", 1}
        ...
        contents[5] = str.clone();
        contents[6] = speed.clone();
        contents[7] = fire.clone();
        ...
        // Slot 19 Sword
        contents[8 + 19] = createEnchanted(Material.NETHERITE_SWORD, new Object[][]{
                {"sharpness", 5},
                {"fire_aspect", 2},
                {"knockback", 1},
                {"unbreaking", 3},
        ...
        // Slot 20 Gapples
        contents[8 + 20] = new ItemStack(Material.GOLDEN_APPLE, 64);

        // Slot 21 Pearls
        contents[8 + 21] = new ItemStack(Material.ENDER_PEARL, 16);

        // 22-23 Strength
        contents[8 + 22] = str.clone();
        contents[8 + 23] = str.clone();

        // 24-25 Speed
        contents[8 + 24] = speed.clone();
        contents[8 + 25] = speed.clone();

        // 26-27 Fire Res
        contents[8 + 26] = fire.clone();
        contents[8 + 27] = fire.clone();
